...
void SendToOutput(FILE *file, const char *fmt, ...) // eliminate it?
{
 ...
 *      for a frag
 ***********************************************************/
void NotFound(FILE *file, const char *term) {
    printf("\nTERM \"%s\" NOT FOUND\n", term);
 ...
 ***********************************************************/
void ReturnResults(const long long position, const long long find_count) {
    if (OUTPUT_STDOUT == 0) {
 ...
 ***********************************************************/
void ReturnTotal(FILE *file, const long long find_count) {
